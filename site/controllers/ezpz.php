 ...
 * users to enter a new ezpz message
 *
 */
class EzpzControllerEzpz extends JControllerForm
{
    ...
    public function __construct($config = array())
    {
        ...
        $this->view_item = $input->get("view", "ezpz", "string");
    ...
    public function cancel($key = null)
    {
        ...
        $this->setRedirect(
            ...
            JText::_(COM_EZPZ_ADD_CANCELLED)
        );
    }

    /*
    * Function handing the save for adding a new ezpz record
    ...
    */
    public function save($key = null, $urlVar = null)
    {
        ...
        if (!JFactory::getUser()->authorise( "core.create", "com_ezpz"))
        {
        ...
        */
        ...
        else
        {
            if ($file['error'] > 0)
            {
                $app->enqueueMessage(JText::sprintf('COM_EZPZ_ERROR_FILEUPLOAD', $file['error']), 'warning');
                return false;
            }
            ...
            if (!isset($file['name']))
            {
                ...
                $app->enqueueMessage(JText::_('COM_EZPZ_ERROR_BADFILENAME'), 'warning');
                return false;
            }
            ...
            $relativePathname = JPath::clean($mediaparams->get(isset($path)?$path:'', 'images') . '/' . $file['name']);
            $absolutePathname = JPATH_ROOT . '/' . $relativePathname;
            if (JFile::exists($absolutePathname))
            {
                // A file with this name already exists
                $app->enqueueMessage(JText::_('COM_EZPZ_ERROR_FILE_EXISTS'), 'warning');
                return false;
            }
            ...
            {
                // Error in upload
                $app->enqueueMessage(JText::_('COM_EZPZ_ERROR_UNABLE_TO_UPLOAD_FILE'));
                return false;
            }
        ...
        // notify the administrator that a new ezpz message has been added on the front end
        ...
        $mailer->setSubject("New ezpz message added by " . $current_username);
        ...
        $this->setRedirect(
            $currentUri,
            JText::_('COM_EZPZ_ADD_SUCCESSFUL')
        );
    ...
    public function getModel($name = '', $prefix = '', $config = array('ignore_request' => true))
    {
        ...
        {
            $input = JFactory::getApplication()->input;
            $modelname = $input->get("modelname", "ezpz", "string");
