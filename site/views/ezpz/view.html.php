...
class EzpzViewEzpz extends JViewLegacy
{
    ...
    function display($tpl = null)
    {
        ...
        $dispatcher->trigger('onContentPrepare', array ('com_ezpz.ezpz', &$item, &$item->params, null));

        $results = $dispatcher->trigger('onContentAfterTitle', array('com_ezpz.ezpz', &$item, &$item->params, null));
        $item->afterDisplayTitle = trim(implode("\n", $results));

        $results = $dispatcher->trigger('onContentBeforeDisplay', array('com_ezpz.ezpz', &$item, &$item->params, null));
        $item->beforeDisplayContent = trim(implode("\n", $results));

        $results = $dispatcher->trigger('onContentAfterDisplay', array('com_ezpz.ezpz', &$item, &$item->params, null));
        ...
        if (!$this->item->canAccess)
        {
            ...
            else
            {
                ...
                $app->enqueueMessage(JText::_('COM_EZPZ_MUST_LOGIN'), 'notice');
        ...
        $this->item->tags = $tagsHelper->getItemTags('com_ezpz.ezpz' , $this->item->id);
    ...
    function addMap()
    {
        ...
        $document->addScript(JURI::root() . "media/com_ezpz/js/openstreetmap.js");
        $document->addStyleSheet(JURI::root() . "media/com_ezpz/css/openstreetmap.css");
